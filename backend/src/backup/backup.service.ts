...
export class BackupService implements OnModuleInit {
  ...
  private get pgBin(): string {
    return process.env.PG_BIN ?? 'C:\\Program Files\\PostgreSQL\\18\\bin';
  }
  ...
  private get uploadsSrc(): string | undefined {
    return process.env.BACKUP_UPLOADS_SRC;
  }
  ...
  async prune(): Promise<void> {
    ...
    const dumps = await this.listByMtime(path.join(this.backupDir, 'full'));
  ...
  private async onFailure(_result: BackupRunResult): Promise<void> {
    // TODO: wire to email / WhatsApp / healthchecks.io
  }
