...
The backup module is already wired into the NestJS app (`AppModule` → `BackupModule`). The following `.env` vars are set:

```
PGHOST=localhost
PGPORT=5432
PGDATABASE=dentaldb2
PGUSER=postgres
PGPASSWORD=Saimen
...
BACKUP_BASE_KEEP=4
# BACKUP_UPLOADS_SRC=C:\Users\Simon\OneDrive\Documents\GitHub\dental-clinic\uploads
# BACKUP_CRON_ENABLED=true
```

Schedules are in `@Cron(...)` decorators in `backend/src/backup/backup.service.ts`: full dump daily 02:00, uploads sync 02:30, prune 04:30, base backup Sunday 03:00.
...
## 2. Enable WAL archiving (the incremental layer) — REQUIRED
...
host  dentaldb2     backup_user  127.0.0.1/32  scram-sha-256
```
...
1. `createdb -U postgres -T template0 dentaldb2_restored`
2. `pg_restore -U postgres -d dentaldb2_restored --no-owner "D:\ClinicBackups\full\full_2026-07-16_020000.dump"`
...
1. Stop the app, then `net stop postgresql-x64-18`.
2. Rename the data directory (default `C:\Program Files\PostgreSQL\18\data`) to `data_broken` — keep it until you're sure.
...
7. `net start postgresql-x64-18` and watch the log folder — you'll see WAL being replayed, then "database system is ready to accept connections".
...
## 7. PostgreSQL 18+ native incremental backups

PG 18 supports native incremental backups: set `summarize_wal = on`, then `pg_basebackup --incremental=<path-to-previous-backup_manifest>` produces a backup containing only changed blocks, and `pg_combinebackup` merges the chain at restore time. It's a valid alternative to WAL archiving if you prefer discrete nightly incrementals over a continuous archive — though WAL archiving still gives you finer recovery granularity (any minute vs. backup times only), which is why it's the default here.
